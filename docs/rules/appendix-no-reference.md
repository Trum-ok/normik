...
| Объявлено в | `nk.rules.appendices.appendix_reference` |
| Фикстуры | `tests/fixtures/appendix-no-reference/` |
| Автоисправление | нет |
...
```toml
disable = ["appendix-no-reference"]

[rules."appendix-no-reference"]
severity = "info"
```
