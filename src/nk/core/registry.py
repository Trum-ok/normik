...
def _applies(impl: RuleImpl, profile: Profile | None) -> bool:
    ...
    по чужому. Универсальное требование применимо всегда, даже если пункт под
    него есть только у одного стандарта: пункты говорят, где записано, а не когда
    проверять. Ключ ``enable`` в профиле сильнее: им подключают требование из
    другого стандарта, если так велит положение вуза.
    """
    if profile is None:
        return True
    return impl.applies_under(profile.standard) or profile.is_enabled(impl.id)
