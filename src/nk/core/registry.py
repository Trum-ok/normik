...
def _applies(impl: RuleImpl, profile: Profile | None) -> bool:
    ...
    по чужому. Типографика и требования чужих положений от выбора стандарта не
    зависят и применимы всегда. Ключ ``enable`` в профиле сильнее: им подключают
    требование из другого стандарта, если так велит положение вуза.
    """
    if profile is None:
        return True
