...
#: Сокращённая форма тоже упоминание: она нарушает и оборот, и полноту слова,
#: и сказать об этом надо сразу, а не после того, как исправят сокращение.
MENTION = re.compile(
    r"(?:\bрисун(?:ок|ка|ку|ком|ке|ки|ков|кам|ками|ках)\b|\bрис\.)"
    r"\s*~?\s*(?:\d|\\(?:ref|autoref|cref))",
    re.IGNORECASE,
)
